 ...
 * A small plugin for getting the CSV of a categorized chart
 Modified to add units, and work properly for multiple series with different periods
 */
(function (Highcharts) {
    ...
    Highcharts.Chart.prototype.getCSV = function () {
        ...
        each (this.series, function (series) {
            if (series.options.includeInCSVExport !== false) {
				...
				//allow quotes and delimeter characters
				seriesName = series.name.replace(/"/g,'""');
				yTitle = series.yAxis.axisTitle.text.replace(/"/g,'""');
				columns[columns.length - 1].unshift('"' + seriesName + "; " + yTitle + '"');
            }
        });
